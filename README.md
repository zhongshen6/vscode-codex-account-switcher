...
- 状态栏现在显示当前账号的每周剩余额度，格式为 `Codex: 账号名 | 周 xx%`
- 当前账号配额改为前台固定 30 秒刷新一次；后台轮询也调整为每 30 秒触发，并在每次触发时更新当前账号
- 打开账号切换窗口时，不再立即刷新当前账号配额，避免与前台定时刷新重复
- 账号切换选择框现在显示套餐、5h/周额度、恢复时间和最近刷新时间，顶部同时显示账号总数
