...
const patchSteps = [
  ...
  },
  {
    ...
    regex: /let ([A-Za-z$_][\w$]*)=new ([A-Za-z$_][\w$]*)\(([\s\S]*?)\);e\.push\(\1\);let ([A-Za-z$_][\w$]*)=new ([A-Za-z$_][\w$]*)\(\1\);if\(e\.push\(it\.window\.registerUriHandler\(\4\)\),e\.push\(it\.window\.registerWebviewViewProvider\(\2\.viewType,\1,\{webviewOptions:\{retainContextWhenHidden:!0\}\}\)\),/,
    replace: (_match, providerVar, providerClassVar, providerArgs, uriHandlerVar, uriHandlerClassVar) =>
      `let ${providerVar}=new ${providerClassVar}(${providerArgs});e.push(${providerVar});let closeCodexEditors=async()=>{let codexEditorTabs=[];for(let codexTabGroup of it.window.tabGroups.all)for(let codexTab of codexTabGroup.tabs){let codexInput=codexTab.input,isCodexEditor=codexInput&&typeof codexInput=="object"&&"viewType"in codexInput&&codexInput.viewType===${providerClassVar}.customEditorViewType;isCodexEditor&&codexEditorTabs.push(codexTab)}codexEditorTabs.length>0&&await it.window.tabGroups.close(codexEditorTabs,!0)},reloadCodexSidebar=async()=>{let codexSidebarWebview=${providerVar}.sidebarView?.webview;if(!codexSidebarWebview)return;${providerVar}.clearPendingRequestsForWebview(codexSidebarWebview),${providerVar}.disposeIpcClientForWebview(codexSidebarWebview),${providerVar}.workerBusMessageHandler.unregisterWebview(codexSidebarWebview),${providerVar}.sidebarWebviewReady=!1,codexSidebarWebview.html='<!DOCTYPE html><html><body style="font-family:sans-serif;padding:16px;color:#888;">Reloading Codex...</body></html>',await new Promise(codexResolve=>setTimeout(codexResolve,150)),await ${providerVar}.initializeWebview(codexSidebarWebview,"sidebar")};let ${uriHandlerVar}=new ${uriHandlerClassVar}(${providerVar});if(e.push(it.window.registerUriHandler(${uriHandlerVar})),e.push(it.window.registerWebviewViewProvider(${providerClassVar}.viewType,${providerVar},{webviewOptions:{retainContextWhenHidden:!0}})),`
  },
  {
    label: "reload-command",
    regex: /(e\.push\(it\.commands\.registerCommand\("chatgpt\.openSidebar",([A-Za-z$_][\w$]*)\)\),e\.push\(it\.commands\.registerCommand\("chatgpt\.openCommandMenu",async\(\)=>\{await it\.commands\.executeCommand\("workbench\.action\.showCommands"\)\}\)\),)(e\.push\(it\.commands\.registerCommand\(([^,]+),async)/,
    replace: (_match, prefix, openSidebarCommandVar, nextRegistrationPrefix) =>
      `${prefix}e.push(it.commands.registerCommand("${PATCH_COMMAND_ID}",async()=>{try{await closeCodexEditors();let codexReloadResult=await reloadCodexProcess();if(!codexReloadResult.success){it.window.showErrorMessage(codexReloadResult.errorMessage??"Failed to restart Codex.");return}await reloadCodexSidebar(),await ${openSidebarCommandVar}(),it.window.setStatusBarMessage("Codex restarted.",3e3)}catch(codexReloadError){it.window.showErrorMessage(codexReloadError instanceof Error?codexReloadError.message:String(codexReloadError))}})),${nextRegistrationPrefix}`
  }
];
